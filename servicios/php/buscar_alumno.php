...
<div class="main">
    ...
    <input class="in-inp" type="text" name="paterno" maxlength="30" value="<?php echo isset($_POST["paterno"]) ? $_POST["paterno"]:""; ?>" required></input><br>
    <label>Apellido Materno:</label><br>
    <input class="in-inp" type="text" name="materno" maxlength="30" value="<?php echo isset($_POST["materno"]) ? $_POST["materno"]:""; ?>" required></input><br>
	    <br>
...
<?php if($_POST){
  ...
  $sql = "SELECT gen.id_aspirante, gen.nombre, gen.paterno, gen.materno, gen.fecha_nacimiento, gen.escuela_procedencia
          ...
          WHERE (upper(gen.nombre) LIKE upper('%$nombre%') OR upper(gen.paterno) LIKE upper('%$paterno%') OR upper(gen.materno) LIKE upper('%$materno%'))
...
<table align="center">
      ...
      <?php if($ans->num_rows > 0){ while ($res = mysqli_fetch_array($ans)) { ?>
        <tr>
